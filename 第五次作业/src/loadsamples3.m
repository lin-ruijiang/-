function Data = loadsamples3(imgpathlistfile, exc_setlabel)
...
for m = 1:total_num
    stImageFilePath  = [imgpathlistfile, num2str(m,'%04d'),'\'];
    dirImagePathList = dir(strcat(stImageFilePath,'*.jpg'));
    for x = 1:2
    stImagePath   = dirImagePathList(x).name;
    i = 2 * (m-1) + x;
    img = im2uint8(imread([stImageFilePath,stImagePath]));
    Data{i}.width_orig    = size(img, 2);
    Data{i}.height_orig   = size(img, 1);
    % Data{i}.img      = img
    % shapepath = strrep(imgpathlist{i}, 'png', 'pts');
    %Data{i}.shape_gt = params.meanshape;
    %shapepath = strcat(imgpathlist{i}(1:end-3), 'pts');
    %Data{i}.shape_gt = double(loadshape([filedir,shapepath]));
    Data{i}.shape_gt = zeros(68,2);
    % Data{i}.shape_gt = Data{i}.shape_gt(params.ind_usedpts, :);
    % bbox     =  bounding_boxes_allsamples{i}.bb_detector; %
    Data{i}.bbox_gt2 = getbbox(Data{i}.shape_gt); % [bbox(1) bbox(2) bbox(3)-bbox(1) bbox(4)-bbox(2)];
    Data{i}.bbox_facedet = face_detect(img);
    Data{i}.isok = 0;
    if (Data{i}.bbox_facedet==[50,100,100,100])
        Data{i}.bbox_facedet = Data{i}.bbox_gt2;
        Data{i}.isok = 1;
    end
    Data{i}.bbox_gt = Data{i}.bbox_facedet;
    ...
    Data{i}.bbox_gt2(1) = Data{i}.bbox_gt2(1) - region(1);
    Data{i}.bbox_gt2(2) = Data{i}.bbox_gt2(2) - region(2);
    Data{i}.bbox_gt(1) = Data{i}.bbox_gt(1) - region(1);
    Data{i}.bbox_gt(2) = Data{i}.bbox_gt(2) - region(2);
    Data{i}.bbox_facedet(1) = Data{i}.bbox_facedet(1) - region(1);
    Data{i}.bbox_facedet(2) = Data{i}.bbox_facedet(2) - region(2);
    % Data{i}.bbox_facedet = Data{i}.bbox_gt2+(rand(1,4)-0.5)*20;
    Data{i}.bbox_gt = Data{i}.bbox_facedet;
    Data{i}.cut = [region(1) region(2)];
